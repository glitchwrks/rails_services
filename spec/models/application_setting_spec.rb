...
RSpec.describe ApplicationSetting do
  ...
  describe 'Boolean settings' do
    it { expect(FactoryBot.build(:boolean_setting, :value => 'true').typecast_value).to be_truthy }
    it { expect(FactoryBot.build(:boolean_setting, :value => 'false').typecast_value).to be_falsey }

    it 'should not allow values other than true and false' do
      boolean_setting = FactoryBot.build(:boolean_setting, :value => 'untrue')
  ...
  describe 'String settings' do
    it { expect(FactoryBot.build(:string_setting, :value => 'a string').typecast_value).to eq 'a string'}
  end

  describe '#find_by_name' do
    let!(:boolean_setting) { FactoryBot.create(:boolean_setting, :value => 'true') }
    let!(:string_setting) { FactoryBot.create(:string_setting, :value => 'the string setting') }
